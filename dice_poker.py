...
def gui():
    ...
    def throws():
        ...
        dice_rerolls = [dice1_check, dice2_check,
                        dice3_check, dice4_check, dice5_check]
        ...
        if len(dice_rerolls) == 0:
            ...
            while iterations < roll_number:
                iterations += 1
                dice[dice_changes[iterations - 1]
    ...
    reroll1 = Checkbutton(pd_frame, text='1',
                          variable=dice1, onvalue=1, offvalue=0)
    reroll1.grid(column=1, row=5)
    reroll2 = Checkbutton(pd_frame, text='2',
                          variable=dice2, onvalue=2, offvalue=0)
    reroll2.grid(column=2, row=5)
    reroll3 = Checkbutton(pd_frame, text='3',
                          variable=dice3, onvalue=3, offvalue=0)
    reroll3.grid(column=3, row=5)
    reroll4 = Checkbutton(pd_frame, text='4',
                          variable=dice4, onvalue=4, offvalue=0)
    reroll4.grid(column=4, row=5)
    reroll5 = Checkbutton(pd_frame, text='5',
                          variable=dice5, onvalue=5, offvalue=0)
